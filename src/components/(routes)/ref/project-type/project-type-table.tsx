...
import ProjectTypeTabActions from "./tab-actions";
import React from "react";
import { useProjectType } from "@/hooks/use-project-type";
import { DeleteProjectType } from "@/api/ref/project-type";

type IProps = {};

const DEFAULT_SEARCH_VALUE = "" as const;

const ProjectTypeTable: React.FC<IProps> = () => {
  ...
  const metaData = getMetaData(ProjectTypes);

  const handleDeleteProjectType = async () => {
    if (deleteModal.visible) {
      setLoading(true);

      const response = await DeleteProjectType(deleteModal.selectedId);
  ...
  const editHandler = (id: number) => {
    router.push(`/ref/project-type/edit-type/${id}`);
  };
