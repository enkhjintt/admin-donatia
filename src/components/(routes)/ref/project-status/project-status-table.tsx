...
import ProjectStatusTabActions from "./tab-actions";
import React from "react";
import { useProjectStatus } from "@/hooks/use-project-status";
import { DeleteProjectStatus } from "@/api/ref/project-status";
...
const ProjectStatusTable: React.FC<IProps> = () => {
  ...
  const metaData = getMetaData(ProjectStatuss);

  const handleDeleteProjectStatus = async () => {
    if (deleteModal.visible) {
      setLoading(true);

      const response = await DeleteProjectStatus(deleteModal.selectedId);
      ...
      if (response.success) {
        ...
        success("Төслийн төлөв амжилттай устлаа!");
  ...
  const editHandler = (id: number) => {
    router.push(`/ref/project-status/edit-status/${id}`);
  };
  ...
  return (
    <>
      <DeleteConfirmationModal
        ...
        onConfirm={handleDeleteProjectStatus}
      />
      {/* tabActions ni table-n deerh haih uildluud */}
      <ProjectStatusTabActions
