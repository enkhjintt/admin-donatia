...
const AddProjectClassForm: React.FC<IProps> = ({}) => {
  ...
  const handleSubmit = async (values: ProjectClassResponse) => {
    ...
    const response = await CreateProjectClass(newData);

    if (response.success) {
      success("Төслийн ангилал амжилттай үүслээ!");

      setTimeout(() => {
        router.push("/ref/project-class");
      }, 1000);
      setTimeout(() => {
        form.resetFields();
        setLoading(false);
...
export default AddProjectClassForm;
