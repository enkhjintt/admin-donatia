...
import {
  ...
  UpdateRefundStatus,
} from "@/api/ref/refund-status";
import {
  useRefundStatus,
  useRefundStatusById,
} from "@/hooks/use-refund-status";

type IProps = {
  id: number;
};

const EditRefundStatusForm: React.FC<IProps> = ({ id }) => {
  const router = useRouter();
  const [form] = Form.useForm();
  const [loading, setLoading] = useState<boolean>(false);
  const { success, error } = useNotification();
  const { mutate } = useRefundStatus();

  const { data, mutate: editMutate } = useRefundStatusById(id);
  useEffect(() => {
    if (data) {
      form.setFieldsValue({
        ner: data.ner,
  ...
  const handleSubmit = async (values: RefundStatusResponse) => {
    ...
    const response = await UpdateRefundStatus(id, newData);

    if (response.success) {
      success("Буцаалтын төлөв амжилттай засагдлаа!");
      mutate();
      editMutate();
...
export default EditRefundStatusForm;
